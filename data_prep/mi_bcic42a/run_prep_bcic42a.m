...
check_create_folder(output_root);
...
eventcodes = {783, 769, 770, 771, 772}; %event label is unknown at this point, but we get it from a different file later
...
for ptcidx=1:sbj_size
    prepdata_cell = [];
    truelabel_cell = [];
    for seshidx=1:length(session_types)
        

        %load gdf using biosig
        gdfl = pop_biosig([root_gdf_folder 'A' num2str(ptcidx, "%02d") session_types{seshidx} '.gdf']);
        
        %load session label for later use
        loadfile = [root_folder 'true_labels/A' num2str(ptcidx, '%02d') session_types{seshidx} '.mat'];
        load(loadfile);
        ...
        truelabel_cell{seshidx} = classlabel;
    end %end of sesh loop
    
    output_path = [output_root '/subj_' num2str(ptcidx) '.mat'];
    save(output_path, 'prepdata_cell', 'truelabel_cell', '-v7.3');
