...
for runset in runset_config['runsets']: # runset name will be used to find data paths etc
    ...
    for sbjidx in np.arange(dataset_config['participants_in_data']): #for each participant
        ...
        if dataset_config['train_test_is_split'] is True:
            ...
            y_test = preproc_y(loaded_data[dataset_config['y_test_varname']], y_is_zeroed=dataset_config['y_is_zeroed'])
        ...
        # Todo: possible further trimming down of time samples
        ...
        ic_model = ShallowFBCSPNet(in_chans=x_tv.shape[1], n_classes=dataset_config['output_classn'],
                                   input_window_samples=x_tv.shape[2])
        # braindecode model input dimension: N, C, T
        # F1 = number of temporal filters, F2= number of pointwise filters, D=Number of spatial filter
        # all three are filter depth parameters
        # kernel length defines the kernel length for temporal kernel (1,X): default 64

        # create torch compatible datasets
        ...
        search_space['module__in_chans'] = [x_tv.shape[1]] # NCT format
        search_space['module__n_classes'] = [dataset_config['output_classn']]
        search_space['module__input_window_samples'] = [x_tv.shape[2]]
        # add train hp search space
        ...
        # instead of neurlanetclass use eegclass
        # skorch_classifier = NeuralNetClassifier(
        skorch_classifier = EEGClassifier(
            module=ic_model,
            #criterion=NLLLoss,
            criterion=nn.CrossEntropyLoss(weight=xentropy_weight),
            optimizer=AdamW,
            optimizer__weight_decay=train_config['weight_decay'],
        ...
        # data torch cast (check for dimensions)
